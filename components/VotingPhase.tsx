...
import { Skull, Gavel, ShieldCheck, ArrowRight, ShieldAlert } from 'lucide-react';
    ...
    return (
      <div className="flex flex-col h-full max-w-md mx-auto p-6 items-center justify-center animate-fade-in">
         <div className="text-center space-y-6 w-full">
            ...
            <div className={`py-10 px-6 rounded-2xl border-4 flex flex-col items-center gap-4 ${
      ...
      <div className="flex-1 grid grid-cols-2 gap-3 overflow-y-auto content-start custom-scrollbar">
        {activePlayers.map((player) => (
          <button
            ...
            className={`relative p-4 rounded-xl border-2 transition-all flex flex-col items-center justify-center gap-2 min-h-[120px] ${
          ...
          >
            <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-xl ${
               selectedId === player.id ? 'bg-red-500 text-white' : 'bg-slate-700 text-slate-300'
            }`}>
              {player.name.charAt(0).toUpperCase()}
            </div>
            <span className={`font-bold text-lg truncate w-full text-center ${
              selectedId === player.id ? 'text-white' : 'text-slate-300'
            }`}>
              {player.name}
            </span>
            {selectedId === player.id && (
